...
export function JournalTab({ journal, uid, isOwnProfile }: JournalTabProps) {
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      await createJournalEntry(uid, { title: title.trim(), content: content.trim(), category, isPublic });
  ...
  // Filter: non-owners only see public entries
      ...
      <div style={{ display: "flex", gap: "0.5rem", alignItems: "center", marginBottom: "0.375rem", flexWrap: "wrap" }}>
        ...
        {isOwnProfile && (
          <button
            ...
            disabled={deleting}
            style={{ marginLeft: "auto", background: "none", border: "none", color: colors.white.ghost, fontSize: "0.75rem", cursor: "pointer", fontFamily: "inherit", opacity: deleting ? 0.5 : 1 }}
