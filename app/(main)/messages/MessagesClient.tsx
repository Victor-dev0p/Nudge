...
function ThreadView({ thread, currentUid, currentName, isDesktop, onBack }: {
  ...
  useEffect(() => { bottomRef.current?.scrollIntoView({ behavior: "smooth" }); }, [messages, partnerIsTyping]);
  ...
  const handleSend = async () => {
    ...
    try {
      if (viewOnce) { await sendViewOnceMessage(thread.id, currentUid, msg, reply ?? undefined); setViewOnce(false); }
      else { await sendMessage(thread.id, currentUid, msg, reply ?? undefined); }
  ...
  const handleReply = useCallback((msg: ChatMessage, senderName: string) => {
  ...
  return (
    <div style={{ flex: 1, display: "flex", flexDirection: "column", height: "100dvh", overflow: "hidden", minWidth: 0 }}>
      ...
      <div style={{ padding: "0.875rem 1.25rem", borderBottom: `1px solid ${colors.obsidian.border}`, display: "flex", alignItems: "center", gap: "0.875rem", background: colors.obsidian.surface, flexShrink: 0 }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <p style={{ fontSize: "0.72rem", color: colors.white.ghost, margin: 0, fontFamily: "var(--font-mono)" }}>
            @{partner?.username}{" "}
            <span style={{ color: partner?.role === "practitioner" ? colors.amber.DEFAULT : colors.lime.DEFAULT }}>
              · {partner?.role === "practitioner" ? "✓ Certified" : "Peer"}
            </span>
          </p>
        </div>
        <button
      ...
      {/* Upload progress */}
      {uploadProgress !== null && (
        <div style={{ padding: "0.375rem 1rem", background: colors.obsidian.surface, borderTop: `1px solid ${colors.obsidian.border}`, flexShrink: 0 }}>
          <div style={{ height: "3px", background: colors.obsidian.border, borderRadius: "100px", overflow: "hidden" }}>
            <div style={{ height: "100%", width: `${uploadProgress}%`, background: colors.lime.DEFAULT, borderRadius: "100px", transition: "width 200ms" }} />
          </div>
          <p style={{ fontSize: "0.7rem", color: colors.white.ghost, margin: "0.25rem 0 0" }}>Uploading... {uploadProgress}%</p>
        </div>
      )}

      {/* Reply preview */}
      {replyTo && (
        <div style={{ padding: "0.5rem 1rem", background: colors.obsidian.elevated, borderTop: `1px solid ${colors.lime.DEFAULT}30`, display: "flex", alignItems: "center", gap: "0.75rem", flexShrink: 0 }}>
          <div style={{ width: "3px", alignSelf: "stretch", background: colors.lime.DEFAULT, borderRadius: "2px", flexShrink: 0 }} />
          <div style={{ flex: 1, minWidth: 0 }}>
            <p style={{ fontSize: "0.7rem", fontWeight: 700, color: colors.lime.DEFAULT, margin: "0 0 0.1rem" }}>↩ {replyTo.senderName}</p>
            <p style={{ fontSize: "0.78rem", color: colors.white.muted, margin: 0, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{replyTo.content}</p>
          </div>
          <button onClick={() => setReplyTo(null)} style={{ background: "none", border: "none", color: colors.white.ghost, cursor: "pointer", fontSize: "1rem", padding: "0.25rem", flexShrink: 0 }}>✕</button>
        </div>
      )}
      ...
      {viewOnce && (
        <div style={{ padding: "0.375rem 1rem", background: `${colors.lime.DEFAULT}10`, borderTop: `1px solid ${colors.lime.DEFAULT}30`, display: "flex", alignItems: "center", gap: "0.5rem", flexShrink: 0 }}>
          <span style={{ fontSize: "0.72rem", color: colors.lime.DEFAULT }}>👁 View once — disappears after recipient reads it</span>
          <button onClick={() => setViewOnce(false)} style={{ marginLeft: "auto", background: "none", border: "none", color: colors.white.ghost, fontSize: "0.7rem", cursor: "pointer" }}>✕</button>
        </div>
      )}
...
// ─── Message Bubble ───────────────────────────────────────────────────────────
function MessageBubble({ message, isMe, currentUid, chatId, senderName, onReply }: {
  message: ChatMessage; isMe: boolean; currentUid: string;
  chatId: string; senderName: string;
  onReply: (msg: ChatMessage, name: string) => void;
}) {
    ...
    >
      ...
      <div style={{ transform: `translateX(${isMe ? -swipeX : swipeX}px)`, transition: swiping ? "none" : `transform 200ms ${motionTokens.easing.spring}`, maxWidth: "72%" }}>
        {editing ? (
          <div style={{ display: "flex", flexDirection: "column", gap: "0.375rem" }}>
            <input autoFocus value={editText} onChange={(e) => setEditText(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter") handleEditSave(); if (e.key === "Escape") setEditing(false); }} style={{ padding: "0.625rem 0.875rem", borderRadius: "12px", border: `1.5px solid ${colors.lime.DEFAULT}60`, background: colors.obsidian.elevated, color: colors.white.DEFAULT, fontSize: "0.875rem", outline: "none", fontFamily: "inherit" }} />
            <div style={{ display: "flex", gap: "0.5rem", justifyContent: "flex-end" }}>
              ...
              <button onClick={handleEditSave} style={{ fontSize: "0.72rem", color: colors.lime.DEFAULT, background: "none", border: "none", cursor: "pointer", fontFamily: "inherit", fontWeight: 700 }}>Save</button>
            </div>
          </div>
        ) : (
      ...
      {menuOpen && !editing && (
        <div ref={menuRef} style={{ position: "absolute", [isMe ? "right" : "left"]: "0", bottom: "calc(100% + 4px)", background: colors.obsidian.elevated, border: `1px solid ${colors.obsidian.border}`, borderRadius: "12px", overflow: "hidden", zIndex: 50, minWidth: "180px", boxShadow: "0 8px 24px rgba(0,0,0,0.4)" }}>
          ...
          {canEdit && <ContextMenuItem label={`✏️ Edit (${editMinsLeft}m left)`} onClick={() => { setEditing(true); setEditText(message.content ?? ""); setMenuOpen(false); }} />}
          <ContextMenuItem label="🗑 Delete for me" onClick={async () => { await deleteForMe(chatId, message.id, currentUid); setMenuOpen(false); }} />
          {isMe && <ContextMenuItem label="🗑 Delete for everyone" danger onClick={async () => { await deleteForEveryone(chatId, message.id); setMenuOpen(false); }} />}
        </div>
      )}
    </div>
  );
...
function ViewOnceReveal({ message, isMe, onExpire }: { message: ChatMessage; isMe: boolean; onExpire: () => void }) {
  ...
  useEffect(() => {
    const iv = setInterval(() => setS((prev) => { if (prev <= 1) { clearInterval(iv); onExpire(); return 0; } return prev - 1; }), 1000);
    return () => clearInterval(iv);
  }, [onExpire]);
  return (
    <div style={{ display: "flex", justifyContent: isMe ? "flex-end" : "flex-start" }}>
      <div style={{ maxWidth: "72%", padding: "0.625rem 0.875rem", borderRadius: "18px 18px 18px 4px", background: colors.obsidian.elevated, border: `1.5px solid ${colors.lime.DEFAULT}60`, position: "relative" }}>
        <div style={{ position: "absolute", top: "6px", right: "10px", fontSize: "0.6rem", color: colors.lime.DEFAULT, fontFamily: "var(--font-mono)", fontWeight: 700 }}>{s}s</div>
        {message.type === "media" && message.mediaUrl
          ? message.mediaType === "image"
...
function ProofCard({ message, chatId, isMe }: { message: ChatMessage; chatId: string; isMe: boolean }) {
  ...
  return (
    <div style={{ maxWidth: "85%", alignSelf: isMe ? "flex-end" : "flex-start", background: colors.obsidian.elevated, border: `1.5px solid ${statusColor}40`, borderRadius: "14px", padding: "0.875rem 1rem", minWidth: "240px" }}>
      <span style={{ fontSize: "0.62rem", fontWeight: 700, color: statusColor, letterSpacing: "0.08em", textTransform: "uppercase" as const, fontFamily: "var(--font-mono)", display: "block", marginBottom: "0.5rem" }}>Proof Card</span>
...
function UserAvatar({ profile, size }: { profile: ParticipantProfile | null; size: number }) {
  ...
  if (profile?.avatarUrl) return <div style={{ width: size, height: size, borderRadius: "50%", overflow: "hidden", flexShrink: 0, border: `1.5px solid ${color}40` }}><Image src={profile.avatarUrl} alt={profile.displayName} width={size} height={size} style={{ objectFit: "cover" }} /></div>;
  return <div style={{ width: size, height: size, borderRadius: "50%", flexShrink: 0, background: `${color}18`, border: `1.5px solid ${color}40`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: size * 0.3, fontWeight: 700, color, fontFamily: "var(--font-mono)" }}>{initials}</div>;
}
